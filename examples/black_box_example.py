...
train_norm, val_norm = data_norm.split(train_ratio=0.7)  # 70% for training, 30% for validation
...
black2_sim = bid.pem(black2, train_norm)             # Objective: Simulation Errors
black2_pred = bid.pem(black2, train_norm, kstep=1)   # Objective: One-Step Prediction Errors


black2_sim_denorm = data_norm.denormalize_state_space(black2_sim.ss)
black2_pred_denorm = data_norm.denormalize_state_space(black2_pred.ss)
...
list_of_models = [black2_sim.ss, black2_pred.ss]
list_of_names = ["2 order black (sim)", "2 order black (one-step)"]

# Plot model performance on validation data
bid.compare(list_of_models, val_norm, model_names=list_of_names, title="Validation (One-Step)", kstep=1)

bid.compare(list_of_models, val_norm, model_names=list_of_names, title="Validation (Simulation)")


# List state space models to compare
list_of_models = [black2_sim_denorm, black2_pred_denorm]
list_of_names = ["2 order black (sim)", "2 order black (one-step)"]
