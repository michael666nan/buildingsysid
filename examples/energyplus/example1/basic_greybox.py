"""
=======================================================================
Example: Grey-Box System Identification for Building Thermal Modeling
=======================================================================
...
to create a thermal model of a building from data. The workflow includes:
  ...
  - Defining two general grey-box model structures (first and second order)
  - Training the model using optimization
  - Validating model performance
  - Analyzing residuals and saving the model

Grey-box models use a mathematical structure based on a priori
...
model_sampling_time = 1800  # seconds
...
# Uncomment the lines below to inspect data
...
# =====================================================================
print("Step 5: Defining grey-box model structure...")
# provide an estimate of the floor area (optional, but makes estimation morerobust)
floor_area=35

# Create an unconstrained first-order LTI (Linear Time-Invariant) grey-box model
grey1 = bid.model_set.grey.First(floor_area=floor_area)   # first-order means it will have 1 internal state

# Create an unconstrained second-order LTI (Linear Time-Invariant) grey-box model
grey2 = bid.model_set.grey.Second(floor_area=floor_area)   # Second-order means it will have 2 internal states
...
opt_problem1 = bid.calculate.OptimizationManager(
    model_structure=grey1,    # The model structure to optimize
...
opt_problem2 = bid.calculate.OptimizationManager(
    model_structure=grey2,    # The model structure to optimize
    data=train,                # Training data
    objective=onestep_pred,    # Prediction objective
    solver=ls_solver           # Optimization solver
)    
...
grey1_opt = opt_problem1.solve(
    initialization_strategies=["grey_box"]  # Start with values typical physical values for building systems
)
...
print("\n--- One-step Predictions (Short-term Forecast) ---")
...
print("\n--- 12-step Predictions (12-hour Forecast) ---")
...
print("\n--- Full Simulation (Long-term Behavior) ---")
_, _ = bid.validation.compare(
...
print("\nGrey-box system identification completed successfully!")